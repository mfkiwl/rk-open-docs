...
通常在开机阶段，U-Boot由前一级的bootloader加载到DRAM的低地址。U-Boot完成board_f.c 的流程后会把自己重定向到内存末尾某个预留的地址上（称为relocation，这个地址根据U-Boot内存布局而定），完成relocation再继续完成board_r.c 流程。

...
步骤1：在kernel reboot前，打印出GRF_OS_REG[0]的值

结果：OS_REG的标志有正常写入

步骤2：kernel一开始打印GRF_OS_REG[0]的值

结果：OS_REG的标志已经变成0
所以，怀疑是在kernel前的流程中被清0了。

步骤3：在uboot一开始打印GRF_OS_REG[0]的值
...
步骤1：因为免拆机的loader是一定版本后才支持的，不知道客户的版本是否支持。所以，先让客户把串口log发过来，我确认了DDR版本，确认这个版本是可以支持的。
...
颗粒型号：H9CKNNNCPTMRPR， H9CCNNNBJTMLAR，H9CKNNNBPTMRLR
...
传递给ddr_sre_2_srx 的参数最好设置成全局变量直接饮用，而不通过参数传递。

# Rockchip RK3326 Linux SDK 发布说明
...
文档主要介绍 Rockchip RK3326 Linux4.19 SDK发布说明，旨在帮助工程师更快上手 RK3326 Linux SDK 开发及相关调试方法。
...
| :----------: | :---------: | :---------------: | :------------: | :-----------: |
|    RK3326    |    2018.02-rc3    |       10       |      3.2      |      4.19      |

 **修订记录**

| **日期**   | **版本** | **作者** | **修改说明** |
| -----------| :-------------- | :------------- | :---------- |
| 2021-05-20 | V1.0.0 | Caesar Wang | 初始发布版本 |
...
本 SDK 支持三个系统分别基于 Buildroot 2018.02-rc3，Yocto 3.2 和 Debian 10 上开发，内核基于 Kernel 4.19，引导基于 U-boot v2017.09，适用于 RK3326 EVB 开发板及基于此开发板进行二次开发的所有 Linux 产品。
...
### RK3326 Linux 通用软件包获取方法

#### 通过代码服务器下载

获取 RK3326 Linux 软件包，需要有一个帐户访问 Rockchip 提供的源代码仓库。客户向瑞芯微技术窗口申请 SDK，同步提供 SSH公钥进行服务器认证授权，获得授权后即可同步代码。关于瑞芯微代码服务器 SSH公钥授权，请参考第 10 节  [SSH 公钥操作说明](# SSH 公钥操作说明)。

RK3326_Linux_SDK 下载命令如下：
...
以 RK3326_LINUX4.19_SDK_V1.5_20210520.tgz 为例，拷贝到该初始化包后，通过如下命令可检出源码：

```shell
mkdir rk3326
tar xvf RK3326_LINUX4.19_SDK_V1.5_20210520.tgz -C rk3326
cd rk3326
...
```
.repo/manifests$ realpath rk3326_linux4.19_release.xml
# 例如:打印的版本号为v1.0.0，更新时间为20210520
# <SDK>/.repo/manifests/rk3326_linux4.19_release_v1.0.0_20210520.xml
```

或者参考工程目录：

```
<SDK>/docs/RK3326/RK3326_Linux4.19_SDK_Note.md
```
...
```
<SDK>/docs/RK3326/Rockchip_RK3326_Hardware_Design_Guide_V1.1_CN.pdf
```

RK3326 EVB 硬件开发指南：

```
<SDK>/docs/RK3326/Rockchip_RK3326_User_Manual_EVB_V1.0_CN.pdf
```
...
进入工程<SDK>/device/rockchip/rk3326 目录：

| 板级配置                       | 说明                                               |
| ----------------------------- | --------------------------------------------------- |
| BoardConfig-rk3326-evb-lp3-v10-k4.19.mk |  适用于 RK3326 EVB V10 开发板  |
| BoardConfig-rk3326-evb-lp3-v10-32bit-k4.19.mk |  适用于 RK3326 EVB V10 开发板，运行32位系统 |

方法1
`./build.sh` 后面加上板级配置文件, 例如：

选择**RK3326 EVB V10**的板级配置：

```shell
./build.sh device/rockchip/rk3326/BoardConfig-rk3326-evb-lp3-v10-k4.19.mk
或
./build.sh device/rockchip/rk3326/BoardConfig-rk3326-evb-lp3-v10-32bit-k4.19.mk
```
...
1. BoardConfig-rk3326-evb-lp3-v10-32bit-k4.19.mk
2. BoardConfig-rk3326-evb-lp3-v10-32bit.mk
3. BoardConfig-rk3326-evb-lp3-v10-k4.19.mk
4. BoardConfig-rk3326-evb-lp3-v10.mk
5. BoardConfig-rk3326-robot64.mk
6. BoardConfig-rk3326-robot64_no_gpu.mk
7. BoardConfig.mk
...
```
...
编译后在 Buildroot 目录 output/rockchip_px30_64_4.19/images下生成 rootfs.ext4。

#### Buildroot 的交叉编译

若需要编译单个模块或者第三方应用，需对交叉编译环境进行配置。交叉编译工具位于 buildroot/output/rockchip_rk3326_64_4.19/host/usr 目录下，需要将工具的bin/目录和 aarch64-buildroot-linux-gnu/bin/ 目录设为环境变量，在顶层目录执行自动配置环境变量的脚本（只对当前控制台有效）：
...
```shell
cd buildroot/output/rockchip_rk3326_64_4.19/host/usr/bin

...
文档主要介绍 Rockchip PX30 Linux4.19 SDK发布说明，旨在帮助工程师更快上手 PX30 Linux SDK 开发及相关调试方法。
...
本 SDK 支持三个系统分别基于 Buildroot 2018.02-rc3，Yocto 3.2 和 Debian 10 上开发，内核基于 Kernel 4.19，引导基于 U-boot v2017.09，适用于 PX30 EVB 开发板及基于此开发板进行二次开发的所有 Linux 产品。
...
以 PX30_LINUX4.19_SDK_V1.0_20210520.tgz 为例，拷贝到该初始化包后，通过如下命令可检出源码：
...
```
<SDK>/docs/PX30/Rockchip_PX30_Hardware_Design_Guide_V1.3_CN_20191206 .pdf
```
...
| 板级配置                       | 说明                                               |
| ----------------------------- | --------------------------------------------------- |
| BoardConfig-px30-evb-ddr3-v10-k4.19.mk |  适用于 PX30 EVB V10 开发板  |
| BoardConfig-px30-evb-ddr3-v11-k4.19.mk |  适用于 PX30 EVB V11 开发板  |
...
若需要编译单个模块或者第三方应用，需对交叉编译环境进行配置。交叉编译工具位于 buildroot/output/rockchip_px30_64_4.19/host/usr 目录下，需要将工具的bin/目录和 aarch64-buildroot-linux-gnu/bin/ 目录设为环境变量，在顶层目录执行自动配置环境变量的脚本（只对当前控制台有效）：
